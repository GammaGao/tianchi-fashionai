...
def parse_args():
    ...
    parser.add_argument('-j', '--workers', dest='num_workers', default=1, type=int,
...
def train():
    ...
    train_data = gluon.data.DataLoader(
        ...
        batch_size=10, shuffle=True, last_batch='discard')
    ...
    for epoch in range(epochs):
        ...
        logging.info(
            '[Epoch %d] Train-acc: %.3f, mAP: %.3f, loss: %.3f | Val-acc: %.3f, mAP: %.3f, loss: %.3f | time: %.1f' %
...
def predict(task):
    ...
    f_out = open('output/%s.csv' % (task), 'w')
    ...
    for path, task, _ in task_tokens:
        ...
        pred_out = ';'.join(["%.4f" % (o) for o in out.asnumpy().tolist()])
